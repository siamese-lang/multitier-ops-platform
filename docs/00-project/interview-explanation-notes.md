...
```text
AWS EC2 기반으로 작업 요청·증빙 파일 관리형 경량 웹 서비스를 WEB/WAS/DB/파일저장소/백업/관측성 계층에 배치하고, 장애와 복구 상황을 로그·지표·명령 결과로 검증한 운영 포트폴리오입니다.
```
...
현재 서비스 구현은 작업 요청 웹 화면, 증빙 파일 업로드·다운로드, failure-lab endpoint까지 보강되어 있습니다. 다만 이 보강된 웹 흐름에 대한 AWS runtime evidence는 다음 검증 단계에서 갱신해야 하므로, 이미 검증 완료된 부분과 구현 완료된 부분을 구분해 설명합니다.
```

## 30-second version

```text
EC2 기반으로 Nginx, Spring Boot WAS, PostgreSQL, NFS, backup node, Prometheus monitoring node를 분리 구성했습니다. 운영 대상 서비스는 작업 요청과 증빙 파일을 관리하는 경량 웹 서비스로, 작업 요청 등록·상태 변경·증빙 파일 업로드·다운로드·DB/file consistency 확인 흐름을 갖고 있습니다. 기존 runtime에서는 WEB/WAS/DB 정상 경로, WAS 장애, DB 서비스 장애, DB metadata와 NFS 파일 일관성, 백업 artifact 생성, 별도 restore-lab 복구를 검증했습니다. 특히 DB 장애에서는 DB host는 Prometheus에서 up=1이지만 PostgreSQL service metric은 active=0이고, /readyz와 API는 503이 되는 evidence로 DB service dependency failure라고 판단했습니다.
```
...
Enhanced service implementation now adds:

```text
9. Work-order web pages.
10. Status-change and action memo workflow.
11. Evidence upload/download workflow.
12. Failure-lab endpoints for WEB/WAS/DB/file-storage observations.
```

The enhanced items above are implementation-complete but still need refreshed AWS runtime evidence.
...
### Q7. Prometheus alert까지 했으면 운영 모니터링이 완성된 건가요?
...
### Q8. 왜 OpenKoda가 아니라 ops-sample-service를 사용했나요?

```text
이 프로젝트의 주제는 특정 오픈소스 서비스를 운영하는 것이 아니라 WEB/WAS/DB/Storage/Backup/Observability 계층을 나누고 장애·복구를 검증하는 것입니다. 그래서 운영 시나리오를 명확히 재현할 수 있는 경량 웹 서비스를 직접 보강했습니다. OpenKoda는 Phase 0에서 workload 후보로 검토했지만, 최종 주제는 OpenKoda가 아닙니다.
```

### Q9. 현재 남은 작업은 무엇인가요?

```text
서비스 구현 baseline은 작업 요청 웹 화면, 증빙 파일 업로드·다운로드, failure-lab endpoint까지 들어간 상태입니다. 남은 작업은 이 보강된 서비스 기준으로 Ansible validation을 준비하고, 한 번의 계획된 AWS runtime window에서 웹 화면, 업로드·다운로드, slow request, DB sleep, restore evidence를 갱신하는 것입니다.
```
...
```text
ops-sample-service는 작업 요청·증빙 파일 관리형 경량 웹 서비스로 보강되었다.
작업 요청 웹 화면, 상태 변경, 조치 메모, 증빙 파일 업로드·다운로드, failure-lab endpoint가 구현되어 있다.
```
...
OpenKoda 운영 프로젝트다.
상용 ITSM을 구현했다.
보강된 웹 서비스 runtime evidence까지 모두 갱신 완료했다.
```
...
```text
1. README.md
   - project identity, service implementation status, and validation boundary
2. apps/ops-sample-service/README.md
   - operated service workflow and endpoints
...
5. docs/04-evidence/evidence-index.md
   - evidence map
6. docs/04-evidence/restore-lab-recovery-validation-2026-07-12.md
   - backup/restore proof
7. docs/04-evidence/observability-alert-validation-2026-07-12.md
...
```text
이 프로젝트는 서비스를 예쁘게 배포한 결과물이 아니라, 운영 작업 요청·증빙 파일 관리형 경량 웹 서비스를 대상으로 장애 상황에서 운영자가 어떤 계층을 확인하고 어떤 evidence로 판단해야 하는지를 검증한 운영 포트폴리오입니다.
```
