...
현재까지 인프라 운영 evidence는 확보되었지만, 운영 대상 서비스는 추가 보강 단계에 있습니다. `ops-sample-service`는 단순 샘플 API가 아니라 **운영 작업 요청과 증빙 파일을 관리하는 경량 웹 업무 서비스**로 완성하는 것을 목표로 합니다.
...
Phase 4 이후에는 Grafana dashboard, Alertmanager, Loki, Kubernetes, HA 구성 실습으로 확장하지 않습니다. 다음 작업은 관측성 기능 추가가 아니라, 운영 대상 서비스인 `ops-sample-service`를 면접에서 설명 가능한 경량 웹 업무 서비스로 보강하는 것입니다.

## 서비스 완성도 보강 범위

서비스 완성도 보강 기준은 아래 문서를 따릅니다.
...
완성 후 면접에서 설명할 수 있어야 하는 내용:

```text
작업 요청 등록, 상태 변경, 조치 메모, 증빙 파일 업로드/다운로드,
PostgreSQL metadata와 NFS file object 분리 저장,
WEB/WAS/DB/File 계층의 장애와 복구 검증
```

이 보강이 완료되기 전까지는 `ops-sample-service`를 완성형 업무시스템이나 상용 서비스처럼 설명하지 않습니다.
...
```text
1. README.md
   - project identity, completed evidence, current service-completion scope
2. docs/00-project/ops-sample-service-completion-scope.md
   - operated service definition and implementation scope
3. docs/00-project/portfolio-summary.md
   - recruiter/interviewer-facing summary
4. docs/04-evidence/evidence-index.md
   - claim-to-evidence map
5. docs/00-project/interview-explanation-notes.md
   - 30-second/2-minute explanation and interview Q&A
6. docs/00-project/submission-description-notes.md
...
| ops-sample-service | 작업 요청·증빙 파일 관리형 경량 웹 업무 서비스로 보강 예정 | 완성 전에는 controlled workload 수준으로만 주장 |
...
서비스 보강 완료 전까지 아직 주장하지 않는 것:

```text
작업 요청·증빙 파일 관리형 웹 서비스가 완성되었다.
웹 화면 기반 업무 흐름을 운영했다.
작업 요청 생성/상태 변경/조치 이력을 검증했다.
증빙 파일 업로드/다운로드 전체 흐름을 검증했다.
WEB/WAS timeout/thread/slow query 문제를 서비스 기능과 연결해 검증했다.
```
...
Focus on ops-sample-service completion: domain/schema -> web UI -> upload/download -> ops dashboard/failure lab -> validation.
```
