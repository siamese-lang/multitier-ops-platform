...
resource "aws_route_table" "private" {
  ...
  tags = {
    ...
    Note = "NAT-enabled runtime route for private app dependency retrieval"
  }
}
...
resource "aws_security_group" "app" {
  ...
  description = "Private app node access from bastion only with NAT egress"
  vpc_id      = aws_vpc.this.id
  ...
  egress {
    description = "Allow outbound traffic through the private subnet NAT route."
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
